    ...
    PangolinPlotterImage::PangolinPlotterImage(std::shared_ptr<PangolinWindow> window, int image_height, int image_width, int format, int dtype)
        : m_window_(std::move(window)),
          m_display_(m_window_->AddDisplay(
              "image",
              pangolin::CreateDisplay().SetBounds(0, 1.0, 0, 1.0, static_cast<float>(image_width) / static_cast<float>(image_height)))),
          m_image_texture_(std::make_shared<pangolin::GlTexture>(image_width, image_height, format, false, 0, format, dtype)) {}
