...
x04::x04(int argc, char **argv) {
    ...
    auto cairo_img = Cairo::ImageSurface::create(Cairo::FORMAT_ARGB32, width, height);
    auto cairo_ctx = Cairo::Context::create(cairo_img);

    // cv::Mat img(height, width, CV_8UC3, cv::Scalar(255, 255, 255));
    // pls->smem(width, height, img.data);
    pls->smema(width, height, cairo_img->get_data());
    // std::cout << cairo_img->get_data() << std::endl;
    pls->spage(0.0, 0.0, width, height, 0.0, 0.0);
    ...
    pls->init();
    pls->font(1);
    pl_cmd(PLESC_DEVINIT, cairo_ctx->cobj());
    ...
    cairo_ctx->paint();
    plot1(0);
    cv::Mat img(height, width, CV_8UC4, cairo_img->get_data(), cairo_img->get_stride());
    ...
    cairo_ctx->paint();
    plot1(1);
    img = cv::Mat(height, width, CV_8UC4, cairo_img->get_data(), cairo_img->get_stride());
    cv::cvtColor(img, img, cv::COLOR_BGRA2BGR);
    cv::imshow("test", img);
    cv::waitKey(0);
