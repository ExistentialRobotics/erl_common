...
namespace erl::common {
    ...
    struct GridMapDrawer2D {
        ...
        explicit GridMapDrawer2D(const std::shared_ptr<GridMapInfo2D> &grid_map_info)
        ...
        DrawSegmentsInplace(
            ...
            long num_segments = starts.cols();
            for (long i = 0; i < num_segments; ++i) {
                Eigen::Vector2i start = grid_map_info->MeterToPixelForPoints(starts.col(i));
                Eigen::Vector2i end = grid_map_info->MeterToPixelForPoints(ends.col(i));
        ...
        DrawRaysInplace(
            ...
            Eigen::Vector2i start_pixel = grid_map_info->MeterToPixelForPoints(start);
            cv::Point2i cv_start_pixel(start_pixel(0), start_pixel(1));

            long num_ray_ends = ends.cols();
        ...
        [[nodiscard]] inline cv::Mat
        DrawRays(const cv::Mat &mat, const cv::Scalar &color, int thickness, const Eigen::Ref<const Eigen::Vector2d> &start, const Eigen::Ref<const Eigen::Matrix2Xd> &ends)
        const {
            cv::Mat result = mat.clone();
        ...
        inline void
        DrawPolylineInplace(const cv::Mat &mat, const cv::Scalar &color, int thickness, bool closed, const Eigen::Ref<const Eigen::Matrix2Xd> &points) const {
        ...
        [[nodiscard]] inline cv::Mat
        DrawPolyline(const cv::Mat &mat, const cv::Scalar &color, int thickness, bool closed, const Eigen::Ref<const Eigen::Matrix2Xd> &points) const {
        ...
        [[nodiscard]] inline cv::Mat
        DrawContour(const cv::Mat &mat, const cv::Scalar &color, int thickness, const Eigen::Ref<const Eigen::Matrix2Xd> &contour) const {
        ...
        void
        DrawContourInplace(cv::Mat &mat, const cv::Scalar &color, int thickness, const Eigen::Ref<const Eigen::Matrix2Xd> &contour) const {

            std::vector<std::vector<cv::Point2i>> points(1);
            long num_points = contour.cols();
        ...
        inline void ShowImage(const std::string& title) const {
