...
namespace erl::common {
    ...
    WrapAnglePi(Dtype angle) {
        angle = std::fmod(angle, 2.0f * static_cast<Dtype>(M_PI));
        if (angle < -static_cast<Dtype>(M_PI)) {
            angle += 2.0f * static_cast<Dtype>(M_PI);
        } else if (angle >= static_cast<Dtype>(M_PI)) {
            angle -= 2.0f * static_cast<Dtype>(M_PI);
        }
        return angle;
        // if (angle < -M_PI) {
    ...
    WrapAngleTwoPi(Dtype angle) {
        angle = std::fmod(angle, 2.0f * static_cast<Dtype>(M_PI));
        if (angle < 0.0f) { angle += 2.0f * static_cast<Dtype>(M_PI); }
        return angle;
    ...
    DegreeToRadian(const Dtype degree) {
        return degree * static_cast<Dtype>(M_PI) / 180.0f;
    }

    template<typename Dtype>
    Dtype
    RadianToDegree(const Dtype radian) {
        return radian * 180.0f / static_cast<Dtype>(M_PI);
    }

    /**
     * Convert azimuth-elevation to 3d direction vector
     * @param azimuth: [0, 2 pi), angle between x-axis and the projection of the direction vector on
     * x-y plane
    ...
    DirectionToAzimuthElevation(
        const Eigen::Ref<const Eigen::Vector3<Dtype>> &direction,
        Dtype &azimuth,
        Dtype &elevation) {
