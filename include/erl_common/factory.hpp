...
namespace erl::common {
    ...
    template<typename Base, bool UniquePtr, typename ... Args>
    class Factory {
        public:
            using BaseType = Base;
            using ArgsType = std::tuple<Args...>;
            using InitFunction = std::function<std::conditional_t<UniquePtr, std::unique_ptr<Base>, std::shared_ptr<Base>>(Args...)>;

        private:
            std::map<std::string, InitFunction> m_class_id_mapping_ = {};

        public:
            inline static Factory<Base, UniquePtr, Args...> &GetInstance() {
                static Factory<Base, UniquePtr, Args...> instance;
                return instance;
            }
            ...
            Register(std::string class_id, InitFunction init_function) {
                ...
                if (m_class_id_mapping_.count(class_id)) {
                    ERL_WARN("{} is already registered.", class_id);
                    return false;
                }
            ...
            Create(const std::string &class_id, Args ... args) {
                ...
                if (it == m_class_id_mapping_.end()) {
                    ERL_WARN("Unknown class id: {}. Here are the registered class ids:", class_id);
