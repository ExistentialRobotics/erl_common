...
namespace erl::common {
    ...
    SaveEigenMatrixToBinaryFile(const std::string &file_path, const Eigen::Ref<const Eigen::MatrixX<T>> &matrix) {
        ...
        if (!ofs.is_open()) { throw std::runtime_error("Could not open file " + file_path); }
        long matrix_shape[2] = {matrix.rows(), matrix.cols()};
        ofs.write(reinterpret_cast<char *>(matrix_shape), 2 * sizeof(long));
    ...
    LoadEigenMatrixFromBinaryFile(const std::string &file_path) {
        ...
        if ((file_size - 2 * sizeof(long)) % sizeof(T) != 0) {
            throw std::runtime_error(
                ERL_FORMAT_STRING("File size is not a multiple of the element size. File size: %ld, element size: %ld", file_size, sizeof(T)));
        }
        long matrix_shape[2];
    ...
    template<erl::common::EigenTextFormat Format, typename Matrix>
    std::string
    EigenToString(const Matrix &matrix) {
        std::stringstream ss;
        ss << matrix.format(erl::common::GetEigenTextFormat(Format));
        return ss.str();
    }
    ...
    EigenToDefaultFmtString(const Matrix &matrix) {
        std::stringstream ss;
        ss << matrix.format(erl::common::GetEigenTextFormat(erl::common::EigenTextFormat::kDefaultFmt));
        return ss.str();
    }
    ...
    EigenToCommaInitFmtString(const Matrix &matrix) {
        std::stringstream ss;
        ss << matrix.format(erl::common::GetEigenTextFormat(erl::common::EigenTextFormat::kCommaInitFmt));
        return ss.str();
    }
    ...
    EigenToCleanFmtString(const Matrix &matrix) {
        std::stringstream ss;
        ss << matrix.format(erl::common::GetEigenTextFormat(erl::common::EigenTextFormat::kCleanFmt));
        return ss.str();
    }
    ...
    EigenToOctaveFmtString(const Matrix &matrix) {
        std::stringstream ss;
        ss << matrix.format(erl::common::GetEigenTextFormat(erl::common::EigenTextFormat::kOctaveFmt));
        return ss.str();
    }
    ...
    EigenToNumPyFmtString(const Matrix &matrix) {
        std::stringstream ss;
        ss << matrix.format(erl::common::GetEigenTextFormat(erl::common::EigenTextFormat::kNumpyFmt));
        return ss.str();
    }
    ...
    EigenToCsvFmtString(const Matrix &matrix) {
        std::stringstream ss;
        ss << matrix.format(erl::common::GetEigenTextFormat(erl::common::EigenTextFormat::kCsvFmt));
        return ss.str();
    }
